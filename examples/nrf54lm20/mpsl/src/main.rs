...
async fn main(spawner: Spawner) {
    ...
    #[cfg(feature = "radio-1m")]
    let phy = MpslRadioPhy::<650, 1600>::new(mode, state);
    #[cfg(not(feature = "radio-1m"))]
    ...
    match ROLE {
        Role::Central => {
            ...
            loop {
                ...
                if now - report_at >= embassy_time::Duration::from_secs(5) {
                    ...
                    info!("BENCH C slots={} tx={} rx={} rloss={}% rate={}/s bw={}B/s rtt_avg={}us rtt_min={}us rtt_max={}us busy={}us rev_lost={} rev_loss={}% dup={} fill={} rxd={} txd={} df={} retx={}", frames, ping_tx, echo_rx, rloss, rate, bw, ra, rmin, rmax, avg_busy, rev_lost, rev_loss, dup, fill_rx, central.rx_data(), central.tx_data(), central.delivery_failures(), central.retransmits());
        ...
        Role::Peripheral => {
            ...
            // True once the current echo content was offered on a slot with
            // TX window space; an overwrite before that is an app-layer
            // loss (ow).
