...
async fn main(_spawner: Spawner) {
    // Clock setup: the HFXO is used for the radio; the net core's LFCLK XTAL
    // is owned by the app core, so use the internal RC instead.
    ...
    loop {
        ...
        let tx: Option<&[u8]> = {
            #[cfg(feature = "host")]
            {
                if !link.tx_window_full() {
                    if let Some(n) = mailbox().take_tx(&mut ipc_tx) {
                        Some(&ipc_tx[..n])
                    } else if tx_phase && !echo_pending {
                        ping_tx += 1;
            ...
            {
                if tx_phase && !echo_pending && !link.tx_window_full() {
                    ping_tx += 1;
                    ping_seq = ping_seq.wrapping_add(1);
                    t_ping_tx = Instant::now();
                    p[4..].copy_from_slice(&ping_seq.to_le_bytes());
        ...
        if now - report_at >= embassy_time::Duration::from_secs(5) {
            ...
            #[cfg(not(feature = "peripheral"))]
            {
                ...
                info!("BENCH C slots={} tx={} rx={} rloss={}% rate={}/s bw={}B/s rtt_avg={}us rtt_min={}us rtt_max={}us busy={}us rev_lost={} rev_loss={}% dup={} fill={} rxd={} txd={}", frames, ping_tx, echo_rx, rloss, rate, bw, ra, rmin, rmax, avg_busy, rev_lost, rev_loss, dup, fill_rx, link.rx_data(), link.tx_data());
            }
            #[cfg(feature = "peripheral")]
            {
